        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
          <div>
            ...
            <input
              ...
              @input="generateCode(property, index)"
              type="text"
              required
              placeholder="Цвет"
...
export default {
  ...
  mounted() {
    // Initialize properties from prop on mount
  ...
  watch: {
    initialProperties: {
      handler(newVal) {
        // Only update if change comes from parent (like after save)
  ...
  methods: {
    ...
    generateCode(property, index) {
      // Only auto-generate for new properties (without id) or if code is empty
